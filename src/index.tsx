import { Action, ActionPanel, Detail, List, showToast, Toast, environment, useNavigation, Icon } from "@raycast/api";
          ...
          <List.Item
            ...
            actions={
              <ActionPanel>
                <Action.Push
                  title="Lock Keyboard"
                  target={<CleaningView duration={item.value} />}
                />
              </ActionPanel>
            }
          />
...
function CleaningView({ duration }: { duration: number }) {
  ...
  useEffect(() => {
    const startBlocker = async () => {
      ...
      const blockerPath = path.join(environment.assetsPath, "KeyboardBlocker.exe");
...
Press **Ctrl + U** at any time to unlock the keyboard.

![Cleaning](https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbmZ5Z2J4YmF4YmF4YmF4YmF4YmF4YmF4YmF4YmF4YmF4YmF4YmF4JmVwPXYxX2ludGVybmFsX2dpZl9ieV9pZCZjdD1n/l2JHVUriDGEtWOx0c/giphy.gif)
      `}
      actions={
        <ActionPanel>
          <Action title="Unlock Now" onAction={() => {
             killBlocker();
             setIsLocked(false);
          }} />
        </ActionPanel>
